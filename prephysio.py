...
for filename in os.listdir('../../raw/physio'):
...
out = pd.read_csv('../../raw/physio/Outcomes-a.txt').set_index('RecordID')['In-hospital_death']
...
def parse_rec(values, masks, evals, eval_masks, dir_):
    ...
    rec = {}
    rec['x'] = np.nan_to_num(values)
    rec['masks'] = masks.astype('int32')
    # imputation ground-truth
    rec['evals'] = np.nan_to_num(evals)
    rec['eval_masks'] = eval_masks.astype('int32')
    rec['deltas'] = deltas
...
def parse_id(id_):
    data = pd.read_csv('../../raw/physio/{}.txt'.format(id_))
    ...
    indices = np.random.choice(indices, len(indices) // 10 * 8)
...
for id_ in patient_ids:
    ...
    try:
        total_data.append(parse_id(id_))
    except Exception as e:
        print(e)
        continue

np.save('../input/physio/physio_missing80.npy', total_data)
